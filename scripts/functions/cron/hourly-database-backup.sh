...
do
	...
	if [ $INSTALL_S3_BACKUP = 1 ] && [ $S3_BUCKET_NAME != PLACEHOLDER ] && [ $HOURLY_S3_DATABASE_BACKUP = 1 ];
	  then
			/usr/local/bin/aws s3 cp "/home/EngineScript/site-backups/$i/database/hourly/$DATABASE_FILE.gz" "s3://$i/backups/database/daily" --storage-class STANDARD
	fi
  ...
  # Remove Old Backups
	find /home/EngineScript/site-backups/$i/database/hourly -type f -mtime +1 | xargs rm -fR
