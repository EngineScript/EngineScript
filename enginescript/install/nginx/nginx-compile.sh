...
./configure \
  ...
  --with-openssl-opt="enable-ec_nistp_64_gcc_128 enable-tls1_3 no-deprecated no-dtls no-nextprotoneg no-psk no-srp no-ssl3-method no-tests no-tls1-method no-tls1_1-method no-weak-ssl-ciphers zlib -ljemalloc -march=native" \
