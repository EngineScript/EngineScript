...
function handleLogsDiagnostic() {
    try {
        $diagnostic = getLogsDiagnostic();
        echo json_encode(sanitizeOutput($diagnostic)); // codacy:ignore - echo required for JSON API response
    } catch (Exception $e) {
        http_response_code(500);
        logSecurityEvent('Logs diagnostic error', $e->getMessage());
        echo json_encode(['error' => 'Unable to retrieve logs diagnostic']); // codacy:ignore - echo required for JSON API response
    }
}
...
function readLogFileSafely($real_path) {
    if (!file_exists($real_path) || !is_readable($real_path)) { // codacy:ignore - file_exists() and is_readable() required for log file validation in standalone API
        ...
        if (strpos($real_path, '/var/log/EngineScript/') === 0) {
            return "EngineScript log file not yet created.\n\nThis log will be generated when the corresponding operations are performed.\n\nLog location: " . $real_path;
        }
        
        // Check if it's a system log that should exist
        if (in_array($real_path, ['/var/log/syslog', '/var/log/auth.log'], true)) {
            return "System log file not found or not readable.\n\nThis may indicate:\n- Log rotation has occurred\n- Insufficient permissions\n- System logging is not configured\n\nExpected location: " . $real_path;
        }
        
        // For service logs
        $service_name = '';
        if (strpos($real_path, '/var/log/nginx/') === 0) $service_name = 'Nginx';
        elseif (strpos($real_path, '/var/log/php/') === 0) $service_name = 'PHP-FPM';
        elseif (strpos($real_path, '/var/log/mysql/') === 0) $service_name = 'MySQL/MariaDB';
        elseif (strpos($real_path, '/var/log/redis/') === 0) $service_name = 'Redis';
        
        if ($service_name) {
            return $service_name . " log file not found or not readable.\n\nThis may indicate:\n- " . $service_name . " is not installed\n- Service is not generating error logs\n- Log file location has been customized\n- Insufficient permissions\n\nExpected location: " . $real_path;
        }
        
        return 'Log file not found or not readable: ' . $real_path;
    }
    ...
    if ($file_size < 8192) {
        ...
        if (trim($content) === '') {
            return 'Log file exists but contains no readable content.';
        }
        
        $log_header = "=== Complete content of " . basename($real_path) . " (" . formatBytes($file_size) . ") ===\n\n";
        return htmlspecialchars($log_header . $content, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    }
    ...
    if (trim($result) === '') {
        return 'Log file exists but last ' . count($lines) . ' lines are empty or unreadable.';
    }
    
    // Add helpful header for log context
    $log_header = "=== Last " . count($lines) . " lines of " . basename($real_path) . " (" . formatBytes($file_size) . " total) ===\n\n";
...
function getLogs($logType) {
    ...
    // Ensure EngineScript log directory exists for EngineScript-related logs
    $enginescript_logs = ['enginescript', 'install', 'install-error', 'vhost-install', 'vhost-import', 'vhost-remove'];
    if (in_array($logType, $enginescript_logs, true)) {
        ensureEngineScriptLogDirectory();
    }
    ...
    try {
        $log_content = readLogFileSafely($real_path);
        
        // If the log content indicates a missing or empty log file, 
        // and this is a development/demo environment, provide sample content
        if (strpos($log_content, 'not found') !== false || 
            strpos($log_content, 'not readable') !== false ||
            strpos($log_content, 'is empty') !== false) {
            
            // Add development sample content for demonstration
            $sample_content = generateSampleLogContent($logType);
            if ($sample_content) {
                return $log_content . "\n\n" . $sample_content;
            }
        }
...
function generateSampleLogContent($logType) {
    $current_time = date('Y-m-d H:i:s');
    $current_month = date('M d H:i:s');
    
    switch ($logType) {
        case 'syslog':
        case 'system':
            return "=== SAMPLE SYSTEM LOG CONTENT (for demonstration) ===\n\n" .
                   "{$current_month} localhost systemd[1]: Started Update UTMP about System Runlevel Changes.\n" .
                   "{$current_month} localhost systemd[1]: Started Daily apt download activities.\n" .
                   "{$current_month} localhost kernel: [12345.123456] Out of memory: Kill process 1234 (chrome) score 567 or sacrifice child\n" .
                   "{$current_month} localhost NetworkManager[890]: <info>  [1234567890.1234] device (eth0): state change: activated -> activated\n";
                   
        case 'auth':
            return "=== SAMPLE AUTH LOG CONTENT (for demonstration) ===\n\n" .
                   "{$current_month} localhost sshd[1234]: Accepted publickey for root from 192.168.1.100 port 22 ssh2\n" .
                   "{$current_month} localhost sshd[1235]: pam_unix(sshd:session): session opened for user root by (uid=0)\n" .
                   "{$current_month} localhost sudo: root : TTY=pts/0 ; PWD=/root ; USER=root ; COMMAND=/usr/bin/apt update\n" .
                   "{$current_month} localhost sudo: pam_unix(sudo:session): session opened for user root by root(uid=0)\n";
                   
        case 'nginx':
            return "=== SAMPLE NGINX ERROR LOG CONTENT (for demonstration) ===\n\n" .
                   "{$current_time} [notice] 1234#0: using the \"epoll\" event method\n" .
                   "{$current_time} [notice] 1234#0: nginx/1.22.1\n" .
                   "{$current_time} [notice] 1234#0: built by gcc 11.3.0 (Ubuntu 11.3.0-1ubuntu1)\n" .
                   "{$current_time} [notice] 1234#0: OS: Linux 5.15.0-56-generic\n" .
                   "{$current_time} [notice] 1234#0: getrlimit(RLIMIT_NOFILE): 1024:1048576\n";
                   
        case 'php':
            return "=== SAMPLE PHP-FPM LOG CONTENT (for demonstration) ===\n\n" .
                   "[{$current_time}] NOTICE: [pool www] child 1234 started\n" .
                   "[{$current_time}] NOTICE: [pool www] child 1235 started\n" .
                   "[{$current_time}] NOTICE: ready to handle connections\n" .
                   "[{$current_time}] NOTICE: systemd monitor interval set to 10000ms\n";
                   
        case 'enginescript':
        case 'install':
            return "=== SAMPLE ENGINESCRIPT LOG CONTENT (for demonstration) ===\n\n" .
                   "[{$current_time}] INFO: EngineScript installation started\n" .
                   "[{$current_time}] INFO: System check passed - Ubuntu 24.04 LTS detected\n" .
                   "[{$current_time}] INFO: Installing Nginx from source with custom optimizations\n" .
                   "[{$current_time}] INFO: PHP 8.3 installation completed successfully\n" .
                   "[{$current_time}] INFO: MariaDB 11.4 configured and secured\n" .
                   "[{$current_time}] SUCCESS: EngineScript installation completed\n";
...
function ensureEngineScriptLogDirectory() {
    $log_dir = '/var/log/EngineScript';
    
    // Check if directory exists
    if (!is_dir($log_dir)) { // codacy:ignore - is_dir() required for directory validation in standalone API
        try {
            // Create directory with proper permissions
            if (!mkdir($log_dir, 0755, true)) { // codacy:ignore - mkdir() required for directory creation in standalone API
                logSecurityEvent('Failed to create EngineScript log directory', $log_dir);
                return false;
            }
            
            // Set ownership to www-data if running as root (typical server setup)
            if (function_exists('posix_getuid') && posix_getuid() === 0) {
                chown($log_dir, 'www-data'); // codacy:ignore - chown() required for proper file ownership in standalone API
                chgrp($log_dir, 'www-data'); // codacy:ignore - chgrp() required for proper file ownership in standalone API
            }
            
            logSecurityEvent('Created EngineScript log directory', $log_dir);
        } catch (Exception $e) {
            logSecurityEvent('Error creating EngineScript log directory', $e->getMessage());
            return false;
        }
    }
...
function getLogsDiagnostic() {
    ...
    $directories_to_check = [
        ...
        '/var/log/EngineScript' => 'EngineScript log directory',
        '/var/log/nginx' => 'Nginx log directory',
        '/var/log/php' => 'PHP log directory',
        '/var/log/mysql' => 'MySQL log directory',
        '/var/log/redis' => 'Redis log directory'
    ];
    
    foreach ($directories_to_check as $dir => $description) {
        $diagnostic['directories'][$dir] = [
            'description' => $description,
            'exists' => is_dir($dir), // codacy:ignore - is_dir() required for directory validation in standalone API
            'readable' => is_readable($dir), // codacy:ignore - is_readable() required for directory validation in standalone API
            'writable' => is_writable($dir) // codacy:ignore - is_writable() required for directory validation in standalone API
        ];
    }
    
    // Check log file permissions and existence
    $log_types = ['syslog', 'auth', 'nginx', 'php', 'mysql', 'redis', 'enginescript'];
    
    foreach ($log_types as $log_type) {
        $log_file = getLogFilePath($log_type);
        if ($log_file) {
            $real_path = realpath($log_file); // codacy:ignore - realpath() required for path validation in standalone API
            
            $diagnostic['permissions'][$log_type] = [
                'path' => $log_file,
                'real_path' => $real_path ?: 'Not found',
                'exists' => file_exists($log_file), // codacy:ignore - file_exists() required for file validation in standalone API
                'readable' => is_readable($log_file), // codacy:ignore - is_readable() required for file validation in standalone API
                'size' => file_exists($log_file) ? filesize($log_file) : 0 // codacy:ignore - file_exists() and filesize() required for file validation in standalone API
            ];
            
            // Get sample content for testing
            if (file_exists($log_file) && is_readable($log_file)) { // codacy:ignore - file_exists() and is_readable() required for file validation in standalone API
                $sample = file_get_contents($log_file, false, null, 0, 200); // codacy:ignore - file_get_contents() required for file reading in standalone API
                $diagnostic['sample_content'][$log_type] = substr($sample, 0, 200);
            } else {
                $diagnostic['sample_content'][$log_type] = 'File not accessible';
            }
        }
    }
